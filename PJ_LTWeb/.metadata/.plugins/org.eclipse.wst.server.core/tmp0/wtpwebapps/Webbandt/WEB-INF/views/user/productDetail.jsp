  		...
  		<div class="col-6">
	  		<div class="mt-5">
	  			...
	  			<div class="mt-4" style="display: inline-flex;">
	            	<h3 class="fw-bolder" style="padding-right: 15px;'">${product.name } </h3>
	            	
            		<h3 class="fw-bolder">${product.specification }</h3>
	            </div>
				...
				<c:choose>
					<c:when test="${not empty message }">
						<button type="button" class="btn btn-primary"><i class="fas fa-cart-plus"></i> Đã thêm vào giỏ hàng</button>
					</c:when>
					<c:otherwise>
						<c:choose>
							<c:when test="${product.quantity > product.sold_quantity }">
								<a href="user/addCart.htm?id=${product.id }"><button type="button" class="btn btn-outline-primary"><i class="fas fa-cart-plus"></i> Thêm vào giỏ hàng</button></a>
							</c:when>
							<c:otherwise>
